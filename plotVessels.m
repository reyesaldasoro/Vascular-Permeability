function  dataOut = plotVessels(dataIn)
%function  dataOut = plotVessels(dataIn);
%
...
if ~ischar(dataIn)
    ...
    if dataIn(1:2)~= 'u_'
        ...
        plotMontages =        findobj(h,'Position', [0.51    0.83    0.48    0.15]);
        axisTimeLines =       findobj(h,'Position', [0.51    0.13    0.48    0.16]);
...
%             end
        ...
        elseif dataIn(3) == '6'
            ...
            %fig;
            clf;
            plotVessels(data.dataIn(minRow:maxRow,minCol:maxCol,:,:));
            clear;
...
%                    lowThresHandle1         = line([data.g_tissueA  data.g_tissueA], [1 data.maxY],'color',[1 0  0],'tag','lowThresHandle1');axis tight
        ...
        elseif dataIn(3) == '9'
            ...
            callBackTimeSamples = ['fig2 = gcf;','fig = fig2-1;','hand3 = get(fig2,''children'');','data = get(fig,''userdata'');',...
                ...
                'set(fig,''userdata'',data);','plotVessels(data);','clear;'];
        ...
        elseif dataIn(3) == 'A'
            ...
            callBackSlices = ['fig2 = gcf;','fig = fig2-1;','hand3 = get(fig2,''children'');','data = get(fig,''userdata'');',...
                ...
                'close(fig2);',...
                'plotVessels(data.dataIn(:,:,data.tSeries,:));','clear;'];
...
h_uimenu_read  =  uimenu(h_uimenu,'Label','Read (mat format)','Callback','[sfile,pathname] = uigetfile(''*.mat'',''Please Indicate file'');if sfile~= 0 plotVessels(strcat(pathname,sfile)); end;');
%h_uimenu_read2  =  uimenu(h_uimenu,'Label','Read (biorad format)','Callback','[sfile,pathname] = uigetfile(''*.PIC'',''Please Indicate file'',''multiselect'',''on'');if size(sfile,2)>= 1 data = readPICs(pathname,sfile);plotVessels(data); end;');
h_uimenu_read2  =  uimenu(h_uimenu,'Label','Read (biorad or images)','Callback',' dataIn=readPermeabilityData();plotVessels(dataIn);');
...
% h_uimenu3_reduce  =      uimenu(h_uimenu3,'Label','** Reduce size (Quad/Oct Tree)',               'Callback','fig = gcf;data = get(fig,''userdata'');data2 = reduceu(data.dataIn);plotVessels(data2);clear;');
% h_uimenu3_rotate  =      uimenu(h_uimenu3,'Label','Transpose data (x<--->y)',                     'Callback','fig = gcf;data = get(fig,''userdata'');data.classA = permute(data.classA,[2 1 3 4]);data.maskD = permute(data.maskD,[2 1 3 4]);data.dataIn = permute(data.dataIn,[2 1 3 4]);temRow = data.rows;data.rows = data.cols;data.cols = temRow;plotVessels(data);clear;');
% h_uimenu3_register1  =   uimenu(h_uimenu3,'Label','** Registration of data (Rigid)',              'Callback','fig = gcf;data = get(fig,''userdata'');data2 = shiftData(data.dataIn,''rigid'');plotVessels(data2);clear;');
% h_uimenu3_register2  =   uimenu(h_uimenu3,'Label','** Registration of data (Regional Rigid)',     'Callback','fig = gcf;data = get(fig,''userdata'');data2 = shiftData(data.dataIn,''nonrigid'');plotVessels(data2);clear;');
% h_uimenu3_zoom  =        uimenu(h_uimenu3,'Label','** Zoom into a region (graph) ',               'Callback','plotVessels(''u_6'');');
% h_uimenu3_zoom2  =       uimenu(h_uimenu3,'Label','** Zoom into a region (coords)',               'Callback','plotVessels(''u_B'');');
% h_uimenu3_equalise  =    uimenu(h_uimenu3,'Label','** Equalise intensity in the slides',          'Callback','fig = gcf;data = get(fig,''userdata'');avIntensity = (sum(sum(data.dataIn(:,:,:,1)))/data.rows/data.cols);avIntensityDiff = avIntensity-avIntensity(1); q = repmat(avIntensityDiff,[data.rows data.cols 1 data.timeSamples]);   data.dataIn = data.dataIn-q;figure(fig+1);plot(squeeze(avIntensity),''b-o'');figure(fig);plotVessels(data);clear;');
% h_uimenu3_warn  =        uimenu(h_uimenu3,'Label','** Warning! these process ** cannot be undone ');


if ~isempty(data)
    % ------ Push Button to End programme
    pbend =  uicontrol(fig,'Style','push','String','End',                                 'position',[10 35 150 30],...
    ...
    pbgraf =  uicontrol(fig,'Style','push','String','Recalculate',                        'position',[10 72 150 30],...
        'CallBack','plotVessels(''u_4'')','Handlevisibility','on','tag','pbgraf','value',0);
    %------ Push Button to  to remove a  mask
    %pbmask =  uicontrol(fig,'Style','push','String','Erase ROI',                          'position',[170 35 110 30],...
    %    'CallBack','plotVessels(''u_3'')','Handlevisibility','on','tag','pbmask','value',0);
    %------ Push Button to  Zoom int a Region of Interest
    %pbzoom =  uicontrol(fig,'Style','push','String','Zoom ROI',                           'position',[285 35 110 30],...
    %    'CallBack','plotVessels(''u_6'')','Handlevisibility','on','tag','pbzoom','value',0);
    ...
    %    'CallBack','plotVessels(''u_C'')','Handlevisibility','on','tag','pbThres','value',0);
    ...
    %    'CallBack','plotVessels(''u_5'')','Handlevisibility','on','tag','pbinclude','value',0,'fontsize',10);
    %------ button to remove time samples
    pbTSamps =  uicontrol(fig,'Style','push','String','Remove samples',                   'position',[170 72 150 30],...
        'CallBack','plotVessels(''u_9'')','Handlevisibility','on','tag','pbTSamps','value',0);

    %------ button to remove SLICES
    pbSlices =  uicontrol(fig,'Style','push','String','Remove Slices',                   'position',[170 35 150 30],...
        'CallBack','plotVessels(''u_A'')','Handlevisibility','on','tag','pbSlices','value',0);
%
    %------  VAlues for the thresholds editable and in %
          uicontrol(fig,'Style','edit','string',num2str(data.g_marker,3),       'position',[10 110 40 22], ...
        ...
        'Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texMarker');
          uicontrol(fig,'Style','edit','string',num2str(data.g_tissue,3),       'position',[10 140 40 22], ...
        ...
        'Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texTissue');
          uicontrol(fig,'Style','edit','string',num2str(data.border),           'position',[150 110 40 22],...
        'callback','','Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texBorder');
           uicontrol(fig,'Style','edit','string',num2str(data.colToPlot),        'position',[150 140 40 22],...
        'callback','plotVessels(''u_1'')','Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texXLine');

    %------  VAlues for the thresholds, non editable and in pix intensity
    texMarkerA              =  uicontrol(fig,'Style','edit','string',num2str(data.g_markerA),                               'position',[100 110 40 22], ...
    ...
    texTissueA              =  uicontrol(fig,'Style','edit','string',num2str(data.g_tissueA),                               'position',[100 140 40 22], ...
    ...
    %------  Text for the names
    nomMarker               =  uicontrol(fig,'Style','text','string','Marker',                                              'position',[55 110 40 22], 'Handlevisibility','on','tag','nomMarker');
    nomTissue               =  uicontrol(fig,'Style','text','string','Tissue',                                              'position',[55 140 40 22], 'Handlevisibility','on','tag','nomTissue');
    nomBorder               =  uicontrol(fig,'Style','text','string','Border',                                              'position',[195 110 40 22], 'Handlevisibility','on','tag','nomBorder');
    nomXLine                =  uicontrol(fig,'Style','text','string','X Line',                                              'position',[195 140 40 22], 'Handlevisibility','on','tag','nomXLine');

    %-------------------------Change the slice that is being viewed --------------------
    %------  Text for the names
    nomSlice                =  uicontrol(fig,'Style','text','string',strcat('slice: (1:',num2str(data.levs),')'),           'position',[295 110 80 22], 'Handlevisibility','on','tag','nomSlice');
    nomTime                 =  uicontrol(fig,'Style','text','string',strcat('Time:  (1:',num2str(data.timeSamples),')'),    'position',[295 140 80 22], 'Handlevisibility','on','tag','nomTime');
    nomRows                 =  uicontrol(fig,'Style','text','string',strcat('Rows:     ',num2str(data.rows)),               'position',[395 110 80 22], 'Handlevisibility','on','tag','nomSlice');
    nomCols                 =  uicontrol(fig,'Style','text','string',strcat('Columns: ',num2str(data.cols)),                'position',[395 140 80 22], 'Handlevisibility','on','tag','nomSlice');
    texSlice                =  uicontrol(fig,'Style','edit','string','1',                                                   'position',[250 110 40 22], 'callback','plotVessels(''u_2'')','Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texSlice');
    texTime                 =  uicontrol(fig,'Style','edit','string','1',                                                   'position',[250 140 40 22], 'callback','plotVessels(''u_2'')','Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texTime');
%%
    %-------------------------Change dimensions of the voxel if necessary --------------------
    %------  Text for the names
    %text(355,85,'\mu','interpreter','tex')

    nomVoxSize              =  uicontrol(fig,'Style','text','string','Dimensions of voxel',                                 'position',[355 85 180 22], 'Handlevisibility','on','tag','nomVoxSize');
    nomVoxSizeR             =  uicontrol(fig,'Style','text','string','Row     : ',                                          'position',[355 60 90 22], 'Handlevisibility','on','tag','nomVoxSizeR');
    nomVoxSizeC             =  uicontrol(fig,'Style','text','string','Column  : ',                                          'position',[355 40 90 22], 'Handlevisibility','on','tag','nomVoxSizeC');
    nomVoxSizeZ             =  uicontrol(fig,'Style','text','string','Z Level : ',                                          'position',[355 20 90 22], 'Handlevisibility','on','tag','nomVoxSizeZ');

    texVoxSizeR             =  uicontrol(fig,'Style','edit','string',num2str(data.dimVox_R),                                'position',[450 60 50 22], 'callback','texVoxSizeR =  findobj(gcf,''tag'',''texVoxSizeR'');set(texVoxSizeR,''background'',[0 1 1]);clear texV*;','Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texVoxSizeR');
    texVoxSizeC             =  uicontrol(fig,'Style','edit','string',num2str(data.dimVox_C),                                'position',[450 40 50 22], 'callback','texVoxSizeC =  findobj(gcf,''tag'',''texVoxSizeC'');set(texVoxSizeC,''background'',[0 1 1]);clear texV*;','Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texVoxSizeC');
    texVoxSizeZ             =  uicontrol(fig,'Style','edit','string',num2str(data.dimVox_Z),                                'position',[450 20 50 22], 'callback','texVoxSizeZ =  findobj(gcf,''tag'',''texVoxSizeZ'');set(texVoxSizeZ,''background'',[0 1 1]);clear texV*;','Handlevisibility','on','BackgroundColor',[1,1,1],'tag','texVoxSizeZ');

    nomVoxSizeR2            =  uicontrol(fig,'Style','text','string','[m','fontname','symbol',                               'position',[505 60 12 22], 'Handlevisibility','on','tag','nomVoxSizeR2');
    nomVoxSizeR3            =  uicontrol(fig,'Style','text','string','m]',                                                   'position',[517 60 15 22], 'Handlevisibility','on','tag','nomVoxSizeR3');
    nomVoxSizeC2            =  uicontrol(fig,'Style','text','string','[m','fontname','symbol',                               'position',[505 40 12 22], 'Handlevisibility','on','tag','nomVoxSizeC2');
    nomVoxSizeC3            =  uicontrol(fig,'Style','text','string','m]',                                                   'position',[517 40 15 22], 'Handlevisibility','on','tag','nomVoxSizeC3');
    nomVoxSizeZ2            =  uicontrol(fig,'Style','text','string','[m','fontname','symbol',                               'position',[505 20 12 22], 'Handlevisibility','on','tag','nomVoxSizeZ2');
    nomVoxSizeZ3            =  uicontrol(fig,'Style','text','string','m]',                                                   'position',[517 20 15 22], 'Handlevisibility','on','tag','nomVoxSizeZ3');
 
%%    
    
    t_vs_pat                =  uicontrol(fig,'Style','popup','string','Intensity Vessel,tissue|Intensity Vessel,tissue,boundaries|Intensity Vessel,tissue,boundaries,Uncertain|Intensity Tissue,Boundaries|Intensity Tissue,boundaries,Uncertain|Patlak|Intensity Tissue/Vessel|Vessel, Tissue + Thresholds','value',1,'units','normalized', 'position',[0.6 0.01 0.28 0.05],  'callback','plotVessels(''u_7'')','Handlevisibility','on','tag','t_vs_pat');
%    t_vs_pat                =  uicontrol(fig,'Style','popup','string','Intensity Vessel,tissue|Intensity Vessel,tissue,boundaries|Intensity Vessel,tissue,boundaries,Uncertain|Intensity Tissue,Boundaries|Intensity Tissue,boundaries,Uncertain|Patlak|Intensity Tissue/Vessel|Vessel, Tissue + Thresholds','value',1, 'position',[550 10 260 25],  'callback','plotVessels(''u_7'')','Handlevisibility','on','tag','t_vs_pat','units','normalized');
    %mask_vs_slice  =  uicontrol(fig,'Style','popup','string','Class mask (row,col)|Mask volume Cut (row,slice)|Data volume Cut (row,slice)|Data time Cut (row,t)|Volume Projection (row,col)|Region Leak (row,col)|Time Leak (row,col)|Time Leak Norm(row,col)|Vessels (3D)|Boundary (3D)|1 Vol Montage(x,y,z)|1 Slice Montage(x,y,t)|Histogram|Data Slice Linear Montage|Mask Slice Linear Montage','value',1, 'position',[750 210 405 25],  'callback','plotVessels(''u_8'')','Handlevisibility','on','tag','mask_vs_slice');
%%
%    mask_vs_slice           =  uicontrol(fig,'Style','popup','string','Class mask [rows,columns]|Mask volume Cut [rows,z position]|Data volume Cut [rows, z position]|Data time Cut [rows,time]|Average Intensity Projection [rows,columns]|Z-stack Intensity [rows,columns,z position]|Vessels (3D) [rows,columns,z position]|Boundary (3D) [rows,columns,z position]','value',1, 'position',[750 210 405 25],  'callback','plotVessels(''u_8'')','Handlevisibility','on','tag','mask_vs_slice','units','normalized');
    mask_vs_slice           =  uicontrol(fig,'Style','popup','string','Class mask [rows,columns]|Mask volume Cut [rows,z position]|Data volume Cut [rows, z position]|Data time Cut [rows,time]|Average Intensity Projection [rows,columns]|Z-stack Intensity [rows,columns,z position]|Vessels (3D) [rows,columns,z position]|Boundary (3D) [rows,columns,z position]','value',1,'units','normalized', 'position',[0.6 0.3  0.28 0.05],  'callback','plotVessels(''u_8'')','Handlevisibility','on','tag','mask_vs_slice');

%%    
%    hist_vs_montage         =  uicontrol(fig,'Style','popup','string','Montage (x,y,z) of one time stack [rows,z position]|Montage (x,y,t) of one slice along time [rows,time] |Histogram  [relative occurrence, voxel intensity]','value',3,'units','normalized', 'position',[750 505 405 25],  'callback','plotVessels(''u_8'')','Handlevisibility','on','tag','hist_vs_montage');
    hist_vs_montage         =  uicontrol(fig,'Style','popup','string','Montage (x,y,z) of one time stack [rows,z position]|Montage (x,y,t) of one slice along time [rows,time] |Histogram  [relative occurrence, voxel intensity]','value',3,'units','normalized', 'position', [0.6 0.756  0.28 0.05],  'callback','plotVessels(''u_8'')','Handlevisibility','on','tag','hist_vs_montage');

%%    
    
   
    %------- this is the histogram and the threshold
    plotMontages            = subplot('Position', [0.51      0.83    0.48    0.15]);
...
%     imagesc(dataTemp)
    %------ Surf of one slice
                            subplot('Position', [0.03   0.38    0.45    0.37]);
    ...
    %------ the classes
    classesAxis          =  subplot('Position', [0.51    0.38    0.48    0.37]);
    ...
    axisTimeLines        = subplot('Position', [0.51    0.13    0.48    0.16]);
...
%     radButtonMulti       = uicontrol(fig,'Style','radio','String','Multiple Time Thresholding','position',[55 180 180 25],'value',0,'tag','radButtonMulti','callback','radButtonSingle = findobj(gcf,''tag'',''radButtonSingle'');set(radButtonSingle,''value'',0);t_vs_pat = findobj(gcf,''tag'',''t_vs_pat'');set(t_vs_pat,''value'',8);plotVessels(''u_7'');');
% 
...
%         'callback','radButtonOriginal  = findobj(gcf,''tag'',''radButtonOriginal''); set(radButtonOriginal,''value'',0);  mask_vs_slice = findobj(gcf,''tag'',''mask_vs_slice'');set(mask_vs_slice,''value'',13);plotVessels(''u_8'');');
...
%     end
    ...
    k1Text           =  uicontrol(fig,'Style','text','position',[820  10 100  30],  'string',strcat('k1 = '    ,num2str(data.patlak(1,4),3)), 'visible','off','tag','k1Text');
    k2Text           =  uicontrol(fig,'Style','text','position',[920  10 100  30],  'string',strcat('k2 = '    ,num2str(data.patlak(2,4),3)), 'visible','off','tag','k2Text');
    k3Text           =  uicontrol(fig,'Style','text','position',[1020 10 100  30],  'string',strcat('k3 = '    ,num2str(data.patlak(3,4),3)), 'visible','off','tag','k3Text');
    permeabText      =  uicontrol(fig,'Style','text','position',[1120 10 130  30],  'string',strcat('P = '    ,num2str(data.permeability,4),' [1e7 cm/s]  PS/V = ',num2str(data.PSV,3),' [1e4 1/s]'),'visible','off','tag','permeabText');
    finVessText      =  uicontrol(fig,'Style','text','position',[820  10 120  30],  'string',strcat('V(',num2str(data.timeSamples),') = '  ,num2str(data.measurementsInTime(end,1),4)), 'visible','on','tag','finVessText');
    finTissText      =  uicontrol(fig,'Style','text','position',[930  10 120  30],  'string',strcat('T(',num2str(data.timeSamples),') = '  ,num2str(data.measurementsInTime(end,2),4)), 'visible','on','tag','finTissText');
    finRatioText     =  uicontrol(fig,'Style','text','position',[1050 10 120  30],  'string',strcat('T/V(',num2str(data.timeSamples),') = '  ,num2str(data.measurementsInTime(end,2)./data.measurementsInTime(end,1),4)), 'visible','on','tag','finRatioText');
